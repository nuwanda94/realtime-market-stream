"""Silver lakehouse sink: Hive-partitioned enriched ticks and bars.

Path layout::

    {root}/silver/ticks/event_type={trade|ohlcv}/symbol={SYM}/date={YYYY-MM-DD}/{batch}.jsonl
"""
...
class SilverRecord:
    """One enriched event ready for the Silver layer."""
    ...
    def partition_path(self) -> str:
        return (
            f"silver/ticks/event_type={self.event_type}/symbol={self.symbol}/date={self.event_date}"
        )
...
class SilverSink(Protocol):
    """Write accepted Silver records to object storage / disk."""

    def write(self, records: list[SilverRecord]) -> list[str]: ...


class FilesystemSilverSink:
...
class InMemorySilverSink:
    ...
    def write(self, records: list[SilverRecord]) -> list[str]:
        ...
        self.writes += 1
        return [f"memory://silver-{self.writes}"]
