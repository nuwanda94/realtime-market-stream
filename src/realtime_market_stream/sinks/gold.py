"""Gold lakehouse sink: Hive-partitioned anomaly-scored bars.

Path layout::

    {root}/gold/bars/symbol={SYM}/date={YYYY-MM-DD}/{batch}.jsonl
"""
...
class GoldRecord:
    ...
    def partition_path(self) -> str:
        return f"gold/bars/symbol={self.symbol}/date={self.event_date}"
...
class GoldSink(Protocol):
    """Write accepted Gold records to object storage / disk."""

    def write(self, records: list[GoldRecord]) -> list[str]: ...


class FilesystemGoldSink:
    """Partitioned JSONL writer used when LAKEHOUSE_FORMAT=jsonl."""
    ...
    def write(self, records: list[GoldRecord]) -> list[str]:
        if not records:
            return []
        groups: dict[str, list[GoldRecord]] = {}
...
class InMemoryGoldSink:
    """Test double that keeps records in process memory."""

    def __init__(self) -> None:
        self.records: list[GoldRecord] = []
        self.writes: int = 0

    def write(self, records: list[GoldRecord]) -> list[str]:
        self.records.extend(records)
        self.writes += 1
        return [f"memory://gold-{self.writes}"]
